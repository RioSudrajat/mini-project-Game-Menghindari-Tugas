...
public class TugasFisdas extends GameObject {

    private Handler handler;

    public TugasFisdas(int x, int y, ID id, Handler handler) {
        super(x, y, id); // Memanggil konstruktor superclass (GameObject) dengan koordinat (x, y) dan ID yang diberikan
        this.handler = handler;
        kecX = 5;       // Mengatur kecepatan awal objek pada sumbu X
        kecY = 5;       // Mengatur kecepatan awal objek pada sumbu Y
    }
    ...
    public void update() {
        ...
        handler.addObject(new jejak((int)x,(int) y,ID.jejak,Color.YELLOW, 16, 16, 0.03f, handler));
    }
    ...
    public void render(Graphics g) {
        ...
        g.setColor(Color.YELLOW); // Mengatur warna untuk menggambar objek musuh
